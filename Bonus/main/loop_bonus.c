...
/*   Updated: 2024/12/07 18:14:50 by aaghla           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render_animations(t_data *data)
{
	...
	if (data->mouse && !data->bigmap)
		handle_mouse(data);
	if ((!data->is_moving) && data->light
		&& !data->turning_on && !data->turning_off)
		anm_idle_light_on(data, &data->idle_light_on_anm, mlx_get_time() * 1e3);
	else if (data->light && data->is_moving
		&& !data->turning_on && !data->turning_off)
		anm_walk_light_on(data, &data->walk_light_on_anm, mlx_get_time() * 1e3);
	else if (!data->light && data->is_moving
		&& !data->turning_on && !data->turning_off)
		anm_walk_light_off(data, &data->walk_light_off_anm,
			mlx_get_time() * 1e3);
}
